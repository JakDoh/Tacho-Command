...
export const DDP_REQUEST_DOWNLOAD_INTERFACE_VERSION = withChecksum([0x80, 0xee, 0xf0, 0x02, 0x36, 0x00]);
export const DDP_REQUEST_GEN2V2_OVERVIEW = withChecksum([0x80, 0xee, 0xf0, 0x02, 0x36, 0x31]);
export const DDP_REQUEST_OVERVIEW = DDP_REQUEST_GEN2V2_OVERVIEW;
...
export const DDP_PHASES = Object.freeze([
  ...
  Object.freeze({ name: "download-interface-version", requestSid: 0x36, positiveSid: 0x76, trep: 0x00, message: DDP_REQUEST_DOWNLOAD_INTERFACE_VERSION }),
  Object.freeze({ name: "gen2v2-overview", requestSid: 0x36, positiveSid: 0x76, trep: 0x31, message: DDP_REQUEST_GEN2V2_OVERVIEW }),
  Object.freeze({ name: "driver-card-slot-1", requestSid: 0x36, positiveSid: 0x76, trep: 0x06, message: DDP_REQUEST_DRIVER_CARD_SLOT_1 }),
...
function validatePhaseParameters(phase, event) {
  if (!event.data || !Array.isArray(event.data)) return true;
  const data = event.data;
  if (phase.name === "start-communication" && data.length >= 3) {
    return data[1] === 0xea && data[2] === 0x8f;
  }
  if (phase.name === "start-diagnostic-session" && data.length >= 2) {
    return data[1] === 0x81;
  }
  if (phase.name === "request-upload" && data.length >= 3) {
    return data[1] === 0x00 && data[2] === 0xff;
  }
  if (phase.name === "download-interface-version") {
    // Expected response format: [0x76, 0x00, counter_hi, counter_lo, 0x02, 0x02]
    if (data.length >= 6) return data[4] === 0x02 && data[5] === 0x02;
    if (data.length >= 3) return data[1] === 0x02 && data[2] === 0x02;
    return false;
  }
  return true;
}
...
export function advanceDdpSession(session, event) {
  ...
  if (event.type === "positive" && event.sid === phase.positiveSid && (phase.trep === undefined || event.trep === phase.trep)) {
    if (!validatePhaseParameters(phase, event)) return session;

...
function decodeTachoString(bytes) {
  ...
  // First byte is codePage index (e.g. ISO 8859-1..16), remaining are characters
...
export function extractCardMetadata(tlvObjects) {
  ...
  // Offsets per Annex 1C / Annex 1B Appendix 2:
  // Offset 0: cardIssuingMemberState (1 byte)
  // Offset 1..16: cardNumber (16 bytes: 14 chars ID + 1 replacement + 1 renewal)
  // Offset 17..52: cardIssuingAuthorityName (36 bytes)
  // Offset 53..56: cardIssueDate (4 bytes timestamp)
  // Offset 57..60: cardValidityBegin (4 bytes timestamp)
  // Offset 61..64: cardExpiryDate (4 bytes timestamp)
  // Offset 65..100: holderSurname (36 bytes)
  // Offset 101..136: holderFirstNames (36 bytes)
...
export function generateOfficialDddFileName(metadata, date = new Date(), isGen2v2 = true) {
  ...
  // Standard extension: .DDD (or .C1C for Gen2v2 smart card)

...
const DEFAULTS = Object.freeze({
  ...
  maxAttempts: 3,
  maxPending: 40,
  cardTimeoutMs: 1200000, // up to 20 minutes for driver-card slot 1
});
...
async function transfer(transport, request, trep, phase, options) {
  ...
  let lastAck = null;
  let pendingCount = 0;
  const isCardSlot1 = phase === "driver-card-slot-1";
  const initialTimeoutMs = isCardSlot1 ? options.cardTimeoutMs : options.p2Ms;

  while (true) {
    const timeoutMs = lastAck === null ? initialTimeoutMs : options.pendingMs;
    const message = await transport.receive(timeoutMs);
    if (!message) {
      if (lastAck === null && isCardSlot1) {
        // Never blindly resend the initial driver-card request on timeout
...
export async function runDdpCardDownload(transport, inputOptions = {}) {
  ...
  try {
    ...
    const versionPayload = await transfer(transport, DDP_REQUEST_DOWNLOAD_INTERFACE_VERSION, 0x00, "download-interface-version", options);
    if (!versionPayload || versionPayload.length < 2 || versionPayload[0] !== 0x02 || versionPayload[1] !== 0x02) {
      throw new DdpRunnerError("unsupported-interface-version", "download-interface-version", versionPayload ? Array.from(versionPayload).map((b) => b.toString(16).padStart(2, "0")).join(" ") : "empty");
    }
